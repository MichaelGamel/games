...
import { BOARD, COURT_DECK, DEFAULT_BANK_RULES, LUCK_DECK, MAX_CARD_CHAIN } from './config'
...
describe('resolveTurn — movement, pass-start and buy options', () => {
  it('reaches the right final tile and pays the start reward exactly once when wrapping', () => {
    const r = roll(makeState([player(0, 38, 1500)]), d(1, 2)) // 38 + 3 → tile 1
  ...
  it('offers a buy only when the property is unowned and affordable', () => {
    ...
    const mine = roll(makeState([player(0, 38, 1500)], { ownership: { 1: own(0) } }), d(1, 2))
    ...
    const broke = roll(makeState([player(0, 3, 50)]), d(1, 1)) // → tile 5, price 100
    expect(broke.buyOption).toBeNull()
  })

  it('charges rent (rentFor) when landing on a rival-owned property', () => {
    const state = makeState([player(0, 38, 1500), player(1, 20, 1500)], { ownership: { 1: own(1) } })
...
describe('resolveTurn — tax', () => {
  it('debits a tax tile with a signed negative delta', () => {
    const tax = roll(makeState([player(0, 2, 1500)]), d(1, 1)) // 2 + 2 → tile 4, income tax 100
    expect(only(tax.effects, 'cash')[0]).toMatchObject({ delta: -100, reason: 'tax' })
  })
})
...
describe('resolveTurn — cards', () => {
  it('chains a luck move card into the next tile in order [card, move]', () => {
    const state = makeState([player(0, 14, 1500)])
    const r = roll(state, d(1, 1), draws('luck', 'advance3')) // → 16 (luck) → +3 → 19 (property)
    expect(kinds(r.effects)).toEqual(['move', 'card', 'move'])
    expect(only(r.effects, 'card')[0]).toMatchObject({ deck: 'luck', cardId: 'advance3' })
    expect(r.finalTile).toBe(19)
    expect(r.buyOption).toEqual({ tile: 19, price: priceOf(19) })
  })

  it('a luck cash card pays out in order [card, cash]', () => {
    const r = roll(makeState([player(0, 4, 1500)]), d(1, 1), draws('luck', 'dividend')) // → 6 (luck)
  ...
  it('a court card draws from the Court deck', () => {
    const r = roll(makeState([player(0, 0, 1500)]), d(1, 1), draws('court', 'inheritance')) // → 2 (court)
  ...
  it('go-to-jail (card) sends to jail (tile 30) with no pass-start and no buy option', () => {
    const r = roll(makeState([player(0, 4, 1500)]), d(1, 1), draws('luck', 'goToJail')) // → 6 (luck) → jail
    expect(only(r.effects, 'jail')[0]).toMatchObject({ seat: 0 })
    expect(only(r.effects, 'passStart')).toHaveLength(0)
    expect(r.finalTile).toBe(30)
    expect(r.buyOption).toBeNull()
  })

  it('the Lucky Club (tile 10) charges a flat fee, no jail', () => {
    const r = roll(makeState([player(0, 8, 1500)]), d(1, 1)) // → tile 10 (lucky club)
    expect(only(r.effects, 'cash')[0]).toMatchObject({ delta: -30, reason: 'club' })
    expect(only(r.effects, 'jail')).toHaveLength(0)
    expect(r.finalTile).toBe(10)
  })

  it('the Fast Bus (tile 20) grants a buff that doubles the next roll', () => {
    const land = roll(makeState([player(0, 18, 1500)]), d(1, 1)) // → tile 20 (fast bus)
  ...
  it('go-to-Start pays the start reward', () => {
    const r = roll(makeState([player(0, 33, 1500)]), d(1, 1), draws('luck', 'gotoStart')) // → 35 (luck) → Start
  ...
  it('collect-each only takes from other solvent, active players', () => {
    ...
    const r = roll(state, d(1, 1), draws('luck', 'birthday')) // → 6 (luck) → collect 50 from each
  ...
  it('pay-each pays every other active player', () => {
    const state = makeState([player(0, 4, 1500), player(1, 20, 100), player(2, 25, 100)])
    const r = roll(state, d(1, 1), draws('luck', 'charity')) // → 6 (luck) → pay 40 to each
    const pay = only(r.effects, 'pay')
  ...
  it('maintenance charges per owned property (−40 × owned), or nothing when none owned', () => {
    const withProps = makeState([player(0, 4, 1500)], { ownership: { 5: own(0), 9: own(0) } })
    const r = roll(withProps, d(1, 1), draws('luck', 'repairs')) // → 6 (luck) → 40 × 2
    expect(only(r.effects, 'cash')[0]).toMatchObject({ delta: -80, reason: 'maintenance' })

    const noProps = roll(makeState([player(0, 4, 1500)]), d(1, 1), draws('luck', 'repairs'))
...
describe('decision / jail-skip builders and helpers', () => {
  it('builds buy / decline decisions from the pending buy', () => {
    const state = makeState([player(0, 0, 1500)], { pendingBuy: { seat: 0, tile: 3, price: 100 } })
    expect(resolveBuyDecision(state)).toEqual({ type: 'decision', seat: 0, tile: 3, action: 'buy', price: 100 })
    expect(resolveDecline(state)).toEqual({ type: 'decision', seat: 0, tile: 3, action: 'decline', price: 100 })
  })
  ...
  it('rentFor honours mortgage and level; helpers read state', () => {
    ...
    const state = makeState([player(0, 0, 1000)], { ownership: { 1: own(0), 3: own(0) } })
    expect(netWorth(0, state)).toBe(1000 + priceOf(1) + priceOf(3))
...
describe('resolveTurn — doubles → extra turn / third → jail (P2)', () => {
  ...
  it('grants an extra turn on doubles with the running count', () => {
    const r = roll(two(), d(2, 2)) // 0 → 4 (income tax), doubles
...
describe('resolveTurn — full-group double rent (P2)', () => {
  ...
  it('charges single rent while the group is incomplete', () => {
    const state = makeState([player(0, 3, 1500), player(1, 0, 1500)], {
      ownership: { 3: own(1), 5: own(1) }, // missing tile 7
    })
...
describe('resolveTurn — richer jail (P2)', () => {
  const jailed = (cash: number, extra: Partial<BankPlayer> = {}) =>
    makeState([player(0, 30, cash, { jailTurns: 3, ...extra }), player(1, 0, 1500)])
  ...
  it('a jailed player who rolls doubles walks free and moves, with no extra turn', () => {
    const r = roll(jailed(1500), d(2, 2)) // doubles → escape, move 4 → tile 34
    expect(only(r.effects, 'jailRelease')[0]).toMatchObject({ seat: 0, via: 'doubles' })
    expect(r.extraTurn).toBe(false)
    expect(r.finalTile).toBe(34)
  })
  ...
  it('a Get-Out-of-Jail card is banked on the seat', () => {
    const r = roll(makeState([player(0, 4, 1500), player(1, 0, 1500)]), d(1, 1), draws('luck', 'getOutOfJail'))
...
describe('resolveTurn — building upgrades raise the rent (P3)', () => {
  // Color group A = tiles 3, 5, 7. Landing on a built-up rival tile pays its
  // level rent, not the (doubled) base.
  it('charges the level rent when the property carries houses', () => {
    const state = makeState([player(0, 3, 1500), player(1, 0, 1500)], {
      ownership: { 3: ownAt(1, 3), 5: own(1), 7: own(1) },
    })
    ...
    const onHouses = makeState([player(0, 1, 1500), player(1, 0, 1500)], {
      ownership: { 3: ownAt(1, 3), 5: own(1), 7: own(1) },
    })
    const hit = roll(onHouses, d(1, 1)) // 1 → 3 (level 3)
    expect(only(hit.effects, 'pay')[0].amount).toBe(BOARD[3].rentByLevel![3])
  })
})

describe('property upgrades — predicates and builders (P3)', () => {
  const groupA = () => ({ 3: own(0), 5: own(0), 7: own(0) }) // Damascus / Beirut / Baghdad
  ...
  it('allows building only with the full group, affordably, and not on a utility', () => {
    ...
    expect(canBuildHouse(s, 0, 3)).toBe(true)

    const partial = makeState([player(0, 0, 1500)], { ownership: { 3: own(0), 5: own(0) } })
    expect(canBuildHouse(partial, 0, 7)).toBe(false)

    const utils = makeState([player(0, 0, 1500)], {
      ownership: { 1: own(0), 8: own(0), 14: own(0), 25: own(0) },
    })
    expect(canBuildHouse(utils, 0, 1)).toBe(false) // utilities are unimprovable

    const broke = makeState([player(0, 0, 10)], { ownership: groupA() })
    expect(canBuildHouse(broke, 0, 3)).toBe(false)
  })

  it('enforces even building (raise the group minimum first) and caps at a hotel', () => {
    const uneven = makeState([player(0, 0, 1500)], { ownership: { 3: ownAt(0, 1), 5: own(0), 7: own(0) } })
    expect(canBuildHouse(uneven, 0, 3)).toBe(false) // already above the group min
    expect(canBuildHouse(uneven, 0, 5)).toBe(true)

    const maxed = makeState([player(0, 0, 1500)], { ownership: { 3: ownAt(0, 4), 5: ownAt(0, 4), 7: ownAt(0, 4) } })
    expect(canBuildHouse(maxed, 0, 3)).toBe(false)
  })

  it('builds a manage resolution with the new level and house cost', () => {
    const s = makeState([player(0, 0, 1500)], { ownership: groupA() })
    expect(buildUpgrade(s, 3)).toEqual({
      type: 'manage',
      seat: 0,
      effects: [{ kind: 'upgrade', seat: 0, tile: 3, level: 1, cost: houseCost(BOARD[3]) }],
    })
  })

  it('sells only from the group maximum, refunding half the house cost', () => {
    const s = makeState([player(0, 0, 1500)], { ownership: { 3: ownAt(0, 2), 5: ownAt(0, 1), 7: ownAt(0, 1) } })
    expect(canSellHouse(s, 0, 3)).toBe(true)
    expect(canSellHouse(s, 0, 5)).toBe(false)
    expect(buildSell(s, 3)).toEqual({
      type: 'manage',
      seat: 0,
      effects: [{ kind: 'sell', seat: 0, tile: 3, level: 1, refund: sellRefund(BOARD[3]) }],
    })
  })
})
...
describe('mortgage — predicates and builders (P4)', () => {
  it('mortgages an unimproved owned property and blocks re-mortgage', () => {
    const s = makeState([player(0, 0, 1500)], { ownership: { 3: own(0) } })
    expect(canMortgage(s, 0, 3)).toBe(true)
    expect(buildMortgage(s, 3)).toEqual({
      type: 'manage',
      seat: 0,
      effects: [{ kind: 'mortgage', seat: 0, tile: 3, amount: mortgageValue(BOARD[3]) }],
    })
    const already = makeState([player(0, 0, 1500)], { ownership: { 3: ownAt(0, 0, true) } })
    expect(canMortgage(already, 0, 3)).toBe(false)
  })

  it('refuses to mortgage while the group carries houses', () => {
    const s = makeState([player(0, 0, 1500)], { ownership: { 3: own(0), 5: ownAt(0, 1) } })
    expect(canMortgage(s, 0, 3)).toBe(false)
  })

  it('lifts a mortgage for the principal + interest when affordable', () => {
    const s = makeState([player(0, 0, 1500)], { ownership: { 3: ownAt(0, 0, true) } })
    expect(canUnmortgage(s, 0, 3)).toBe(true)
    expect(unmortgageCost(BOARD[3])).toBe(Math.round(mortgageValue(BOARD[3]) * 1.1))
    expect(buildUnmortgage(s, 3)).toEqual({
      type: 'manage',
      seat: 0,
      effects: [{ kind: 'unmortgage', seat: 0, tile: 3, cost: unmortgageCost(BOARD[3]) }],
    })
    const broke = makeState([player(0, 0, 5)], { ownership: { 3: ownAt(0, 0, true) } })
    expect(canUnmortgage(broke, 0, 3)).toBe(false)
  })

  it('collects no rent on a mortgaged property', () => {
    const state = makeState([player(0, 38, 1500), player(1, 0, 1500)], {
...
describe('trading — validity and builder (P4)', () => {
  const base = () =>
    makeState([player(0, 0, 1500), player(1, 0, 1500)], { ownership: { 3: own(0), 21: own(1) } })

  it('accepts a legal property-for-property trade', () => {
    const offer: TradeOffer = { from: 0, to: 1, giveTiles: [3], giveCash: 0, receiveTiles: [21], receiveCash: 0 }
  ...
  it('refuses to trade a tile whose group carries houses', () => {
    const s = makeState([player(0, 0, 1500), player(1, 0, 1500)], {
      ownership: { 3: own(0), 5: ownAt(0, 1), 21: own(1) },
    })
    expect(canTrade(s, { from: 0, to: 1, giveTiles: [3], giveCash: 0, receiveTiles: [21], receiveCash: 0 })).toBe(false)
  })
})
