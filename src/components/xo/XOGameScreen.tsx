...
import type { XOController } from '../../hooks/useTicTacToe'
import { XOBoard } from './XOBoard'
import { RoomBadge, type OnlineMeta } from '../online/RoomBadge'
import { LanguageSwitcher } from '../LanguageSwitcher'
import { cn } from '../../lib/cn'

interface XOGameScreenProps {
  game: XOController
  online?: OnlineMeta
}

/**
 * The in-match Tic-Tac-Toe screen — a compact column: header, the two player
 * chips, the grid, and an action bar. Mirrors the layout language of the other
 * game screens.
 */
export function XOGameScreen({ game, online }: XOGameScreenProps) {
  const { t } = useTranslation(['xo', 'common', 'online'])
  const myId = game.controlsPlayer === 'all' ? null : game.controlsPlayer
  const canPlace = game.canPlace && (!online || online.canPlay) && !(game.currentPlayer?.isBot ?? false)
    ...
    >
      ...
      <header className="text-center">
        <h1 className="text-2xl font-bold tracking-tight text-white drop-shadow sm:text-3xl">
          <span aria-hidden="true">✕</span> {t('xo:title')} <span aria-hidden="true">◯</span>
        </h1>
      ...
      <div className="flex w-full max-w-sm items-center justify-between gap-3 rounded-2xl bg-white/5 p-3 ring-1 ring-white/10 backdrop-blur">
        <button
          type="button"
...
const PlayerChips = memo(function PlayerChips({
  ...
  const { t } = useTranslation(['xo', 'common'])
  return (
    <ul className="flex w-full max-w-sm gap-2" aria-label={t('common:setup.players')}>
      {game.players.map((p) => {
        ...
        return (
          ...
          >
            <span
              className="grid h-6 w-6 shrink-0 place-items-center rounded-md text-sm font-black"
              style={{ color: '#0b1020', background: p.color }}
              aria-hidden="true"
            >
              {p.mark}
            </span>
            <span className="min-w-0 flex-1 truncate text-sm font-semibold text-white">
              ...
              {myId === p.id && (
                <span className="ms-1 text-xs font-normal text-white/50">{t('common:game.you')}</span>
              )}
            </span>
            {absent && (
...
function statusText(
  game: XOController,
  t: TFunction<['xo', 'common', 'online']>,
  online?: OnlineMeta,
): string {
  if (game.phase === 'won') {
    if (game.draw) return t('xo:status.draw')
    return game.winner ? t('xo:status.wins', { name: game.winner.name }) : ''
  }
  if (online && !online.canPlay) return t('online:status.waitingReconnect')
  const cur = game.currentPlayer
  if (!cur) return ''
  if (game.phase === 'placing') return t('xo:status.places', { name: cur.name })
  if (online) {
    return game.isMyTurn ? t('xo:status.yourTurn') : t('xo:status.waitingFor', { name: cur.name })
  }
  return t('xo:status.turnPick', { name: cur.name })
}
